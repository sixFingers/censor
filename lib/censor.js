...
function rejectLibrary() {
  var inject = exec('rm -f ./inject.js');
}
...
function runTestSession() {
  ...
  meteor.stdout.on('data', function(data) {
    if(data.toString().indexOf('tests.finish') > 0) {
      process.stdout.write(data.toString().replace('tests.finish', ''));
      process.exit();
    } else {
